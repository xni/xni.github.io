...
var s_account = "bgc3gatesnotesdev"//qa report suite
var s = s_gi(s_account)
/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.dynamicAccountSelection = true
s.dynamicAccountList = "bgc3gatesnotesdev=thegatesnotes.bluerooster.com;bgc3gatesnotesstage=ppe.thegatesnotes.com;bgc3gatesnotesprod=www.gatesnotes.com;bgc3gatesnotesprod=http://gatesnotes.com"
/* Link Tracking Config */
s.trackDownloadLinks = true
s.trackExternalLinks = false
s.trackInlineStats = true
s.linkDownloadFileTypes = "exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx"
s.linkInternalFilters = "javascript:,gatesnotes.com"
...
function s_doPlugins(s) {
    ...
    s.clickPast(s.firstPage, 'event3', 'event4');
    ...
    /* Plugin Example: getPercentPageView v1.5 multiple values*/
    var ppvArray = s.getPercentPageViewed(s.pageName);
    s.prop6 = ppvArray[0] //contains the previous page name
    s.prop7 = ppvArray[1] //contains the total percent viewed

    /* To send the hit on an exit link multiple values*/
    var url = s.exitLinkHandler()
    if (url) {
        s.linkTrackVars = 'prop6,prop7';
        var ppvArray = s.getPercentPageViewed(s.pageName);
        s.prop6 = ppvArray[0]
        s.prop7 = ppvArray[1]
    }
    ...
    s.prop8 = s.getTimeParting('n', '-8'); //use classifications to group the time and the day of week into weekends
    if (s.prop8) s.eVar2 = "D=c8";
...
s.trackingServer = "metrics.gatesnotes.com"
...
//  Clicktracking handlers to help generate manual heatmaps
//  Update if header or footer changes in the future

$(document).on('mousedown', 'a', '', function () {
    try {
        var that = this;
        var mod2, lt, hr, evts, jfooter, jheader;
        var mod = "main";
        var rsid = window.s.fun;
        var lnktype = "o";
        hr = this.href;
        lt = $(this).text().replace(/\s+/gi, " ").slice(0, 99);  // take first 100 chars only of linktext

        if (s.pageName == "home") {
            // handle homepage where data-clickmodule is supported 
            mod = $(this).closest("[data-clickmodule]").attr("data-clickmodule");
            if (mod == undefined) {
                mod = "unknown"
            }
            //console.log("mod: "+mod);
        } else {
            jfooter = $(this).parents("#footer");
            jheader = $(this).parents("#socbanner, #bannerbgmob, #header_topnavBar, #header_subnavBar");
            if (typeof jfooter !== undefined && jfooter.length) {
                mod = "footer";
            } else if (typeof jheader !== undefined && jheader.length) {
                mod = "header";
            }
        }
        s.linkTrackVars = "prop12,prop13,prop14,prop15,prop16,prop17,prop18,prop19,prop20,events";
        ...
        s.prop16 = mod + "__" + lt;
        s.prop17 = mod + "__" + hr;
        s.prop18 = s.pageName + ":" + mod + "__" + lt;
        s.prop19 = s.pageName + ":" + mod + "__" + hr;
        s.prop20 = "D=pageName";
        if (hr && ~hr.indexOf("http") && hr.indexOf("gatesnotes") == -1) {
            //lnktype="e";
            s.linkTrackEvents = "event25,event5";
            s.linkTrackVars = s.apl(s.linkTrackVars, 'eVar3', ',', 2);
            s.events = s.apl(s.events, 'event5', ',', 2);
            s.eVar3 = hr;
        }
        s.tl(that, lnktype, 'mod: ' + mod);
    } catch (e) { console.log("s.tl error:" + e); }
});