...
(function ($) {
	...
	LAPI.Ajax.injectSpinner = function (position, id) {
		spinnerStatusByID[id] = true;
		mw.loader.using('jquery.spinner', function () {
			if (spinnerStatusByID[id]) {
				var $sp = $.createSpinner({
					size: 'small',
					type: 'inline'
				});
				spinnersByID[id] = $sp.attr('id', id).insertAfter(position);
			}
		});
	};
	LAPI.Ajax.removeSpinner = function (id) {
		var $sp = spinnersByID[id];
		if ($sp) $sp.remove();
		spinnerStatusByID[id] = false;
	};
}(jQuery));
